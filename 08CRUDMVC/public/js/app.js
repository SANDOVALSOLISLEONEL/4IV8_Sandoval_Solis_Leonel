...
const inputJugadorNivel = document.getElementById('jugador-nivel');
...
async function cargarSelectJugadores() {
    ...
    try {
        ...
        selectPartidaJugador.innerHTML =
...
async function editarJugador(id) {
    ...
    try {
        ...
        const resp =
            await fetchAPI(`/api/fortnite/jugadores/${id}`);

        inputJugadorId.value = resp.data.id;
        inputJugadorNombre.value = resp.data.nombre;
        inputJugadorNivel.value = resp.data.nivel;
...
formJugador.addEventListener('submit', async (e) => {
    ...
    const datos = {
        ...
        nivel : parseInt(inputJugadorNivel.value)

    };

    try {

        if(inputJugadorId.value){

            await fetchAPI(
                `/api/fortnite/jugadores/${inputJugadorId.value}`,
                {
                    method:'PUT',
                    headers:{
